...
`yaml` meta-data, using the {{#each blog}} command to automatically loop
over each of the above entries in the `data/blog.yml` file, using the
same double-curly-bracket syntax from zurb’s
<a target="_blank" rel="noopener noreferrer"
