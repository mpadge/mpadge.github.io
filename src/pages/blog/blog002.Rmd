...
only knows about `SEXP`, itself a template for a generic "S-EXPression"; and
(ii) All `SEXP` objects are mere pointers to C arrays, which is why R neither
can nor does make any distinction between single values and vectors -- all
R objects are vectors at heart. This suggests something like the following
...
`SEXP`. The trick is to have an additional "type-selector" function that
...
above code (and presuming all to be in a file `src.cpp`) permits a generic
`SEXP`-in / `SEXP`-out function (our `rcpp_mysquare` in the above code) to be
deployed to specific types, and then simply passed to a generic C++ template
function. The whole thing then works like this:
